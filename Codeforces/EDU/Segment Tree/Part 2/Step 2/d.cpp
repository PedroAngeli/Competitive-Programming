...
struct SegTree{
  ...
  void propagate(int pos,int l,int r){
    ...
    st[2*pos+1] += (lazy[pos] * (m-l+1))/(r-l+1);
    st[2*pos+2] += (lazy[pos] * (r-(m+1)+1))/(r-l+1);
    lazy[2*pos+1] += (lazy[pos] * (m-l+1))/(r-l+1);
    lazy[2*pos+2] += (lazy[pos] * (r-(m+1)+1))/(r-l+1);
    lazy[pos] = 0;
  }
